...
class InternalPriorResolver(AbstractNamespaceResolver):
    def can_resolve(self, link_tag: LinkTag) -> bool:
        if not link_tag.is_internal:
            return False
        return link_tag.link_type is LinkType.INTERNAL_PRIOR
    ...
    def resolve(self, link_tag: LinkTag) -> tuple[bool, LinkTag]:
        ...
        if not raw_target:
            raise InvalidNamespaceError("Empty prior namespace.")

        levels_up, remainder = self._parse_prior_prefix(raw_target)

        base_folder: Path = self.context
        for _ in range(levels_up):
            base_folder = base_folder.parent

        try:
            base_folder.relative_to(self.root_folder)
        except ValueError:
            raise InvalidNamespaceError(f"Resolved base escapes root: '{base_folder}'")
