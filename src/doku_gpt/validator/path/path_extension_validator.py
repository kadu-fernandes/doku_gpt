...
class PathExtensionValidator:
    @classmethod
    def validate(cls, path: str | Path, extension: str | ValidFile | None = None) -> Path:
        file_path = Path(str(path).strip()).resolve(strict=False)
        suffix = file_path.suffix.lstrip(".").lower()

        if extension is not None:
            expected = extension.value if isinstance(extension, ValidFile) else ValidFile.validate(str(extension))
            if suffix != expected:
