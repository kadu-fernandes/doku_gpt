...
class TestLinkTagInternalAbsolute(unittest.TestCase):
    def __init__(self, methodName: str = "runTest"):
        super().__init__(methodName)
        self.__link_type: LinkType = LinkType.INTERNAL_ABSOLUTE
        self.__target_prefix: str = ":root:namespace:page"
        self.__target_fragment: str = "section_intro"
        self.__target_query: str = "q=1&x=abc"  # no leading '?'
        self.__resolved_value: str = "root/namespace/page.txt"
        self.__label_value: str = "Absolute Page"
        self.__core_base: str = ":root:namespace:page"
        ...
        self.__root_folder_path: Path = Path("/tmp/wiki")
    ...
    def test_path_when_root_attached_and_resolved_present(self) -> None:
        link_tag = self.__create_valid_link_tag_with_all_fields()
        link_tag.attach_root(self.__root_folder_path)
        expected_path = self.__root_folder_path.joinpath(self.__resolved_value)
        self.assertEqual(expected_path, link_tag.path)
    ...
    def test_path_is_none_when_root_not_attached(self) -> None:
        link_tag = self.__create_valid_link_tag_with_all_fields()
        # no attach_root call
        self.assertIsNone(link_tag.path)

    def test_url_is_none_for_internal_absolute(self) -> None:
        link_tag = self.__create_valid_link_tag_with_all_fields()
        self.assertIsNone(link_tag.url)
